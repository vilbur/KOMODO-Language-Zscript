...
"""A Code Intelligence Language Engine for the Zscript language.
...
Module Usage:
    from cile_zscript import scan
    mtime = os.stat("bar.zscript")[stat.ST_MTIME]
    content = open("bar.zscript", "r").read()
    scan(content, "bar.zscript", mtime=mtime)
"""
...
class ZscriptCILEError(CILEError):
    pass
...
log = logging.getLogger("cile.zscript")
...
def scan_buf(buf, mtime=None, lang="Zscript"):
    """Scan the given ZscriptBuffer return an ElementTree (conforming
    to the CIX schema) giving a summary of its code elements.
    
    @param buf {ZscriptBuffer} is the Zscript buffer to scan
    ...
    """
    # Dev Notes:
    # - This stub implementation of the Zscript CILE return an "empty"
    #   summary for the given content, i.e. CIX content that says "there
    #   are no code elements in this Zscript content".
    # - Use the following command (in the extension source dir) to
    #   debug/test your scanner:
    #       codeintel scan -p -l Zscript <example-Zscript-file>
    ...
    # Dev Note:
    # This is where you process the Zscript content and add CIX elements
    ...
    #   Use the "codeintel html -b <example-Zscript-file>" command as
    #   a debugging tool.
    # - "buf.accessor.text" is the whole content of the file. If you have
    #   a separate tokenizer/scanner tool for Zscript content, you may
    #   want to use it.
